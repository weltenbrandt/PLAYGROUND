...
function __init_debug(){
    globalvar DEBUG;
    globalvar SEND_LOG;SEND_LOG = false;//Requires sentry extension
    DEBUG = {
...
function Trace(_text,_loglevel=LOGLEVEL.TRACE) {
    ...
    if(DEBUG.logging){
        ...
        var _timetxt=TimeFormat(date_current_datetime());
...
function TracePop(_text) {
    Trace(_text);
    if MOBILE    run_notification_pop(_text);
...
function TraceException(_exception,_message) {
    if !is_undefined(_message) Trace(_message,LOGLEVEL.EXCEP);
    ...
    if(SEND_LOG){
        sentry_capture_exception(_txt,SENTRY_FATAL);
    }else{
        Trace(_txt,LOGLEVEL.EXCEP);
    }
}
